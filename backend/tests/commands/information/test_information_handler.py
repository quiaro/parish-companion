...
class TestHomeCallback:
    def test_encodes_the_language_and_home_action(self):
        assert information._home_callback("es") == "info|es|home"
...
class TestParseCallback:
    ...
    def test_home_callback_round_trips(self):
        assert information.parse_callback("info|en|home") == ("en", "home", None)
...
class TestHandleCommand:
    ...
    def test_one_button_row_per_topic_in_the_order_given_plus_a_trailing_home_row(self):
    ...
    def test_english_menu_uses_label_en(self):
        ...
        reply = information.handle_command(_adapter(topics), "en")
        assert reply.button_rows[:-1] == [[("A", "info|en|topic|a")]]  # type: ignore[index]

    def test_spanish_menu_uses_label_es(self):
        topics = [_topic(key="a", label_en="A", label_es="La A", order=1)]
        reply = information.handle_command(_adapter(topics), "es")
        assert reply.button_rows[:-1] == [[("La A", "info|es|topic|a")]]  # type: ignore[index]
    ...
    def test_topic_missing_label_es_is_hidden_from_spanish_menu(self):
        ...
        reply = information.handle_command(_adapter(topics), "es")
        assert reply.button_rows[:-1] == [[("La B", "info|es|topic|b")]]  # type: ignore[index]
    ...
    def test_menu_always_ends_with_a_home_button(self):
        topics = [_topic(key="a", label_en="A", order=1)]
        reply = information.handle_command(_adapter(topics), "es")
        assert reply.button_rows[-1] == [  # type: ignore[index]
            (get_string("information_button_home", "es"), "info|es|home")
        ]
    ...
    def test_shows_apology_when_all_topics_are_hidden_from_spanish_menu(self):
        ...
        assert reply.button_rows == [[(get_string("information_button_home", "es"), "info|es|home")]]

    def test_shows_apology_message_with_only_a_home_button_when_there_are_no_topics(self):
        reply = information.handle_command(_adapter([]), "en")
        assert reply.text == get_string("information_empty", "en")
        assert reply.button_rows == [[(get_string("information_button_home", "en"), "info|en|home")]]

    def test_apology_message_is_localized(self):
        reply = information.handle_command(_adapter([]), "es")
        assert reply.text == get_string("information_empty", "es")

    def test_shows_unavailable_message_with_only_a_home_button_when_fetch_fails(self):
        adapter = MagicMock()
